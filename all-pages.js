(function () {
    ...
    function refreshKorzinaFrame() {
        var panel = document.getElementById("page-korzina");
        if (!panel) return;
        var frame = panel.querySelector("iframe");
        if (!frame) return;
        function tryRender() {
            try {
                if (frame.contentWindow && typeof frame.contentWindow.homieCartRender === "function") {
                    frame.contentWindow.homieCartRender();
                    return true;
                }
            } catch (err) {}
            return false;
    ...
    function getActiveRoute() {
        var active = document.querySelector(".page-panel.active");
    ...
    function show(id) {
        ...
        var prev = getActiveRoute();
        if (id === "korzina" && prev !== "korzina") {
            try {
                sessionStorage.setItem("homiePrevRoute", prev);
            } catch (err) {}
        }
        ...
        document.body.classList.toggle("route-frame", id !== "home");
        history.replaceState(null, "", "#" + id);
        window.scrollTo(0, 0);
        if (id === "korzina") refreshKorzinaFrame();
    }

    window.addEventListener("message", function (e) {
        if (e.data && e.data.type === "homie-cart-updated") {
            refreshKorzinaFrame();
        }
    });
    ...
    (function initHomeSlider() {
        ...
        var people = [
            ...
            { name: "АЛЕКС ХОМИН", role: "SMM-менеджер", quote: "Лучший контент рождается в хаосе", nick: "alex_homie" },
